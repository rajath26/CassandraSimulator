...
public:
   ...
   inline double effective_n_given_fpp(int n, int exp) {
	   cout<<endl<<"Salt size: "<<salt_.size();
	   return std::pow(1.0 - std::exp(-1.0 * salt_.size() * n / size()), (exp));
   }
   ...
   unsigned int getInsertedElementCount() {
	   return inserted_element_count_;
   }
